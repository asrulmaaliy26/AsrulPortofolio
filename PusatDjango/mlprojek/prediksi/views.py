...
def prediksi_svr(request, file_name):
    ...
    if response.status_code != 200:
        return JsonResponse({"error": response_data.get("error")}, status=response.status_code)
    ...
    if request.method == "POST":
        ...
        if fitur and target and target in df.columns:
            ...
            metadata = {"original_file_name": original_file_name, "fitur": fitur, "target": target, "mae": mae, "mape": mape, "r2": r2, "predicted_value": predicted_value, "total_rows": total_rows}
